...
def label_grains(image, image_pred, dbs_max_dist=20.0):
    ...
    props = regionprops_table(
        labels_simple, intensity_image=image, properties=(
            "label", "area", "centroid")
    )
    ...
    if len(label_counts[1:]) > 0:
        ...
        if len(grain_data) > 0:
            coords = np.vstack(
                (grain_data["centroid-1"].values,
                 grain_data["centroid-0"].values)
            ).T
            # Create a DBSCAN clustering object
            dbscan = DBSCAN(eps=dbs_max_dist, min_samples=2)
            # Fit the data to the DBSCAN object
            dbscan.fit(
                np.vstack((grain_data["centroid-1"],
                          grain_data["centroid-0"])).T
            )
...
def predict_large_image(
    ...
    for i in range(0, img_height - patch_size + step_size + 1, step_size):
        for j in range(0, img_width - patch_size + step_size + 1, step_size):
            ...
            patch_num = (
                i // step_size * ((img_width - patch_size +
...
def get_area_weighted_distribution(grain_sizes, areas):
    ...
    for grain_size, area in zip(grain_sizes, areas):
        area_weighted_grain_size.extend(
            [grain_size] * int(area / (0.5 * mean_area)))
