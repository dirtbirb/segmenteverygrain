...
<img src="gravel_example_mask.png" width="600">
...
#### Usage
 The original notebook (above) can be executed directly, but results will not be wholly compatible with the features below. Compatible results can be generated by running the automatic detection and segmentation process through the GUI, or by editing the [run_auto.py](run_auto.py) script for appropriate input/output locations and running it directly. The script only implements the normal image processing method from the notebook, while the GUI includes both the normal and large-image methods.
...
#### Usage
After using segmenteverygrain to generate a grain info csv, generate a grid count through the GUI or directly editing and running [run_point_count.py](run_point_count.py).
